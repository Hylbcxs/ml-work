...
def train(args, model, experiment):
    if args.output_size == 90:
        log_file = f"{args.log_file}/short/{args.loss_type}"
    elif args.output_size == 365:
        log_file = f"{args.log_file}/long/{args.loss_type}"
    else:
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if (epoch + 1) % args.print_every == 0:
            ...
            if args.output_size == 90:
                save_path = f"{args.save_path}/short/{args.loss_type}"
            elif args.output_size == 365:
                save_path = f"{args.save_path}/long/{args.loss_type}"
...
def draw(args, npList, label, experiment):
    ...
    legend = ['Global_active_power', 'Global_reactive_power', 'Voltage', 'Global_intensity', 'Sub_metering_1', 'Sub_metering_2', 'Sub_metering_3','RR','NBJRR1','NBJRR5' ,'NBJRR10', 'NBJBROU', 'temperature_2m_mean']
    for i in range(13):
        # 创建一个新的子图
        if i >= 9:
    ...
    plt.tight_layout()
    if args.output_size == 90:
        savefig_path = f"{args.figure_path}/short/{args.loss_type}"
    elif args.output_size == 365:
        savefig_path = f"{args.figure_path}/long/{args.loss_type}"
